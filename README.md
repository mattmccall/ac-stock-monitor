...
| **Vente-unique.lu** | JSON-LD `ItemList` category (`/c/climatiseur-monobloc`) + product pages (plain HTTP) | Marketplace. Sourced from the **monobloc** category, not the mixed `/c/climatiseur-mobile` (which lists wall-mounted inverter splits — Comfee CF, Aermec, Daikin — alongside real portables). Tiles give name/`/p/` URL. **Scope filter** keeps only portable/monobloc units — drops "mural"/"split" (unless split-mobile/portasplit), the mural-unit brands (Daikin/Emura/Siesta/Baxi Sidera/Aermec/Candy Pura/Aufit), evaporative coolers, **tent/outdoor coolers** ("tente"/"extérieur" — you can't vent a hose from a tent), and pure dehumidifiers. BTU is captured where the page lists it (feeds the underpowered flag); it is *not* a hard gate, because several real monoblocs here (Olimpia Unico, CODILAM) list no BTU while a tent cooler did. Per **product page** (JSON-LD `Product`): price (viewed-product price, sale-aware, ignoring recommended items), `availability` (schema.org InStock/OutOfStock), and delivery lead time where shown → `delivery_days`. Lower-priority breadth; isolated. |
